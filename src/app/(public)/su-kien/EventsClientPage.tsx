...
export default function EventsClientPage({ preSelectedSlug, initialData = {} }: EventsClientPageProps) {
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedTab, setSelectedTab] = useState<'all' | 'upcoming' | 'past'>('all');
  const [events, setEvents] = useState<EventItem[]>(initialData.events || (defaultEvents as EventItem[]));
  ...
  useEffect(() => {
    async function loadEvents() {
      if (initialData && Object.keys(initialData).length > 0) {
        // Handle deep link logic on mount even if data is pre-fetched
        try {
          const params = new URLSearchParams(window.location.search);
          const eventId = params.get('id');
          const eventSlug = preSelectedSlug || params.get('slug');
          if (eventId || eventSlug) {
            const match = events.find(e => eventId ? e.id === eventId : (e.slug === eventSlug || toSlug(e.title) === eventSlug));
            if (match) {
