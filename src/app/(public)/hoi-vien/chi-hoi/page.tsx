            ...
            if (Array.isArray(parsed)) {
              ...
              setMembers(
                chMembers.map((d: any) => ({
                  ...
                  association_role: d.association_role || 'Hội viên chính thức',
                  chapter_role: d.chapter_role,
                  join_date: d.join_date ? (typeof d.join_date === 'string' ? d.join_date.split('T')[0] : new Date(d.join_date).toISOString().split('T')[0]) : (d.created_at ? (typeof d.created_at === 'string' ? d.created_at.split('T')[0] : new Date(d.created_at).toISOString().split('T')[0]) : ''),
                  logo_url: d.logo_url || d.license_file_url,
                  representative_avatar_url: d.representative_avatar_url || '',
                  chapter_id: d.chapter_id,
                }))
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-3 bg-[#fcf9f5] p-4 border border-outline-variant/30 rounded-lg">
                  ...
                  <div className="flex gap-4 items-start pt-1">
                    <div className="w-14 h-14 rounded-full overflow-hidden border border-outline-variant/40 bg-white shrink-0 p-1 flex items-center justify-center">
                      <img
                        src={selectedMember.representative_avatar_url || DEFAULT_HOBA_LOGO}
                        alt="Representative"
                        className="w-full h-full object-contain rounded-full"
                        onError={(e) => { (e.target as HTMLImageElement).src = DEFAULT_HOBA_LOGO; }}
                      />
                    </div>
